...
class BaseOcrModel(BasePageModel, BaseModelWithOptions):
    """An abstract base class for Optical Character Recognition (OCR) models.

    This class provides a common framework for OCR models, including methods for
    identifying regions to be OCRed, filtering the results to avoid overlap
    with existing text, and combining the OCR text with the programmatic text
    from the document.

    Attributes:
        enabled: A boolean indicating if the model is enabled.
        options: An `OcrOptions` object containing the configuration for the OCR model.
    """
    ...
    def get_ocr_rects(self, page: Page) -> List[BoundingBox]:
        """Determines the optimal rectangular regions on a page to perform OCR on.

        This method analyzes the layout of a page to identify areas that are
        likely to contain text that is not programmatically extractable (i.e.,
        text in bitmap images). It merges nearby bitmap regions to create
        larger, more efficient rectangles for OCR.

        Args:
            page: The `Page` object to analyze.

        Returns:
            A list of `BoundingBox` objects representing the areas to be OCRed.
        """
        from scipy.ndimage import binary_dilation, find_objects, label
    ...
    def post_process_cells(self, ocr_cells: List[TextCell], page: Page) -> None:
        """Post-processes the OCR cells and updates the page object.

        This method takes the cells generated by an OCR model, filters out any
        that overlap with existing programmatic text, and then combines the
        remaining OCR cells with the original cells. The page's `textline_cells`
        attribute is then updated with the final, merged list.

        Args:
            ocr_cells: The list of `TextCell` objects from the OCR model.
            page: The `Page` object to be updated.
        """
    ...
    def draw_ocr_rects_and_cells(self, conv_res, page, ocr_rects, show: bool = False):
        """Generates a debug image visualizing the OCR process.

        This method draws the identified OCR regions and the resulting text cells
        (both programmatic and OCRed) on an image of the page. This is useful
        for debugging the OCR pipeline.

        Args:
            conv_res: The `ConversionResult` for the current document.
            page: The `Page` object that was processed.
            ocr_rects: The list of `BoundingBox` objects that were sent to OCR.
